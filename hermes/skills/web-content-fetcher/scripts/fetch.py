...
def fetch_stealth(url, max_chars=30000, timeout=30000):
    ...
    try:
        ...
        if missing not in {"patchright", "playwright"}:
            raise
    ...
    class SimplePage:
        ...
        def css(self, selector):
            ...
            root = html.fromstring(self.html_content)
            matches = root.cssselect(selector)

            class ElementWrapper:
                def __init__(self, node):
                    self.html_content = html.tostring(node, encoding="unicode")

            return [ElementWrapper(node) for node in matches]
    ...
    return extract_content(SimplePage(page_html), url, max_chars)
